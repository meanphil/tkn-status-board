...
$RFLibs::LircMonitor::VERSION = '1.01';
...
sub new
{
	...
	$self->[1]->{events} = [];
	$self->[1]->{history} = [];
	$self->[1]->{lastpresstime} = 0;
	$self->[1]->{sequencetimeout} = 0+$options{sequencetimeout};
...
sub processLine
{
	...
	#if a sequencetimeout option was set, we also wipe this history when no presses occur for the specified period of time
	if ($count == 0)
	{
		if ($self->[1]->{sequencetimeout} > 0)
		{
			my $elapsed  = $time - $self->[1]->{lastpresstime};
			$self->[1]->{lastpresstime} = $time;

			undef(@{$self->[1]->{history}}) if $elapsed > $self->[1]->{sequencetimeout};
		};

		unshift(@{$self->[1]->{history}}, $button);
